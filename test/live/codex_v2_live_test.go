...
func assertCodexLiveV2Compaction(t *testing.T, requests []codexLiveRequest) {
	...
	for requestIndex, request := range requests {
		if request.v2Compaction {
			...
			if !request.v2FunctionPairComplete ||
				!containsCodexLiveInputType(request.inputTypes, "function_call") ||
				!containsCodexLiveInputType(request.inputTypes, "function_call_output") {
				t.Fatal("v2 compaction request has no complete function call/output pair")
			}
