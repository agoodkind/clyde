...
const (
	jsonlConversationID    = "44444444-4444-4444-8444-444444444444"
	composerConversationID = "55555555-5555-4555-8555-555555555555"
	workspaceHash          = "cursor-smoke-workspace"
	searchAttemptCount     = 10
...
func TestCursorSemanticSmokeIngestAndRetrieve(t *testing.T) {
	...
	if err != nil {
		t.Skipf("lm-semantic-search engine unavailable: %v", err)
	}
	...
	createCursorGlobalDB(t, filepath.Join(dataDir, "globalStorage", "state.vscdb"), phrase+" composer")
	createCursorWorkspaceDB(t, dataDir)

	docs, manifest, expectedIDs := collectCursorSemanticDocs(t, ctx, phrase)
	assertProbeDocs(t, docs, expectedIDs, phrase)

	collectionID := fmt.Sprintf("clyde-cursor-smoke-%s", token)
...
func cleanupCursorSmokeCollection(
	...
	for conversationID := range expectedIDs {
		jobID, err := client.DeleteConversation(ctx, collectionID, conversationID)
		if err != nil {
...
func writeCursorJSONLTranscript(
	...
	content := fmt.Sprintf(
		"{\"role\":\"user\",\"message\":{\"content\":[{\"type\":\"text\",\"text\":\"%s\"}]}}\n",
		firstUserText,
	)
...
func createCursorGlobalDB(t *testing.T, dbPath string, firstUserText string) {
	t.Helper()
	...
	insertCursorValue(
		...
		`{"_v":3,"type":1,"bubbleId":"composer-user","text":"`+firstUserText+`"}`,
	)
}

func createCursorWorkspaceDB(t *testing.T, dataDir string) {
	t.Helper()
