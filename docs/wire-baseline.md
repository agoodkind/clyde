...
We never hand-write that wire shape, we learn it from the genuine article. When you run real `claude` or `codex` through Clyde's MITM proxy (see [Cursor](cursor.md) for how that traffic is routed), the proxy records the request's wire shape for the baseline: the headers, the beta flags, and the body field set, with secrets stripped and no prompt text kept in the baseline. The full request and response bodies live separately in the SQLite capture store at `mitm/capture.db`; the baseline itself holds only the field set. That recording becomes a per-upstream baseline, one learned identity per model, and it keeps itself current as the real clients change. There is nothing to configure and no snapshot to maintain by hand.
