...
Reuse the existing metadata declaration in `raw_compaction_protocol.go` and
extend it only when the classifier needs a field that it does not already
carry. Do not declare a second `rawResponsesCompactionMetadata` type.
