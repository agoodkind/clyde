...
generation, and status. Never store encrypted content or credentials. Arm the
entry before Clyde exposes the byte-identical v2 response to the client, then
release it if delivery fails. The process-local staging window is intentional:
restart, expiry, and eviction fail open rather than persisting plaintext
recovery state.
