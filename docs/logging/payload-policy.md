...
Raw capture is separate from the inline payload view. `logging.raw_capture.enabled = true` permits raw request and response sidecar files where a sink supports them. `logging.raw_capture.enabled = false` prevents raw payload sidecar files. Normal process logs, concern logs, request logs, and inventory output do not inline raw payload bodies. The MITM capture index `capture.jsonl` and the `mitm_capture` sink were removed, and MITM wire legs now land in the per-concern file `logs/providers/mitm/wire.jsonl` via the concerns sink.
