...
func TestListBackgroundComposersReturnsWindowMappingsAndSkipsMalformedRows(t *testing.T) {
	...
	statements := []string{
		...
		`INSERT INTO ItemTable(key, value) VALUES ('backgroundComposer.malformed', '{')`,
	}
...
func TestListBackgroundComposersSkipsMalformedWindowMapping(t *testing.T) {
	dbPath := createCursorStoreTestDatabase(t)
	writable, err := sql.Open("sqlite3", "file:"+dbPath+"?_busy_timeout=5000")
	if err != nil {
		t.Fatalf("sql.Open returned error: %v", err)
