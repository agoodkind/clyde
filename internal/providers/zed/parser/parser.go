...
func (p *Parser) Discover(ctx context.Context, _ map[string]conversation.Record) ([]conversation.ScanCandidate, error) {
	...
	for _, root := range roots {
		...
		metadataByThread, err := readMetadataForRoot(ctx, root)
		if err != nil {
			return nil, err
		}
		...
		for _, row := range rows {
			...
			path := BuildVirtualPath(root.RootDir, metadata.Channel, row.ThreadID)
			if path == "" {
				continue
			}
...
func parsableThreadRows(ctx context.Context, path string, rows []zedstore.ThreadRow) []zedstore.ThreadRow {
	...
	for _, row := range rows {
		...
		slog.WarnContext(ctx, "providers.zed.parser.parse_thread_payload_failed", "concern", concern, "path", path, "thread_id", row.ThreadID, "data_type", string(row.DataType), "err", err)
	}
	return filtered
}
