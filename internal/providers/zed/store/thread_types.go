...
// ThreadDocument is the typed view of one current-version Zed thread payload
// after any needed transport-level decode step.
...
// CurrentThreadDecodeError reports one JSON decode failure for a current Zed
// thread payload.
type CurrentThreadDecodeError struct {
	Err error
}

// Error renders the current-thread decode failure.
func (err CurrentThreadDecodeError) Error() string {
	return fmt.Sprintf("decode current zed thread json: %v", err.Err)
}

// Unwrap exposes the underlying JSON decode failure.
func (err CurrentThreadDecodeError) Unwrap() error {
	return err.Err
}
...
func ParseCurrentThreadJSON(data []byte) (ThreadDocument, error) {
	...
	if err := json.Unmarshal(data, &thread); err != nil {
		return ThreadDocument{}, CurrentThreadDecodeError{Err: err}
	}
	if thread.Version != CurrentThreadVersion {
		return ThreadDocument{}, fmt.Errorf("unsupported current zed thread version %q, want %q", thread.Version, CurrentThreadVersion)
	}
	return thread, nil
}
...
func decodeVariant(data []byte) (variantKind, json.RawMessage, error) {
	...
	if err := json.Unmarshal(data, &variants); err != nil {
		return variantKind(""), nil, VariantDecodeError{Err: err}
	}
	if len(variants) != 1 {
...
// VariantDecodeError reports one failure while decoding a tagged Zed enum map.
type VariantDecodeError struct {
	Err error
}

// Error renders the variant decode failure.
func (err VariantDecodeError) Error() string {
	return fmt.Sprintf("decode zed enum variant: %v", err.Err)
}

// Unwrap exposes the underlying JSON decode failure.
func (err VariantDecodeError) Unwrap() error {
	return err.Err
}
