...
// ThreadVersionDecodeError reports one failure while decoding the top-level
// version envelope from a persisted Zed thread payload.
type ThreadVersionDecodeError struct {
	Err error
}

// Error renders the version-envelope decode failure.
func (err ThreadVersionDecodeError) Error() string {
	return fmt.Sprintf("decode zed thread version: %v", err.Err)
}

// Unwrap exposes the underlying JSON decode failure.
func (err ThreadVersionDecodeError) Unwrap() error {
	return err.Err
}

// LegacyThreadDecodeError reports one failure while decoding a legacy Zed
// thread payload.
type LegacyThreadDecodeError struct {
	Err error
}

// Error renders the legacy-thread decode failure.
func (err LegacyThreadDecodeError) Error() string {
	return fmt.Sprintf("decode legacy zed thread json: %v", err.Err)
}

// Unwrap exposes the underlying JSON decode failure.
func (err LegacyThreadDecodeError) Unwrap() error {
	return err.Err
}
...
func detectThreadVersion(data []byte) (string, error) {
	...
	if err := json.Unmarshal(data, &envelope); err != nil {
		return "", ThreadVersionDecodeError{Err: err}
	}
...
func parseLegacyThreadJSON(data []byte) (legacyThread, error) {
	...
	if err := json.Unmarshal(data, &thread); err != nil {
		return legacyThread{}, LegacyThreadDecodeError{Err: err}
	}
	for _, message := range thread.Messages {
		if message.Role != legacyRoleUser && message.Role != legacyRoleAssistant {
			return legacyThread{}, fmt.Errorf("decode legacy zed thread json: unsupported legacy role %q", message.Role)
		}
	}
	return thread, nil
}
...
func upgradeLegacyThread(thread legacyThread) ThreadDocument {
	...
	for _, message := range thread.Messages {
		if canAttachLegacyToolResults(message, messages) {
			...
			if !legacyMessageHasMeaningfulText(message) {
				continue
			}
		}
...
func canAttachLegacyToolResults(message legacyMessage, messages []ThreadMessage) bool {
	return message.Role == legacyRoleUser && len(message.ToolResults) > 0 && len(messages) > 0 && messages[len(messages)-1].Agent != nil
}

func legacyMessageHasMeaningfulText(message legacyMessage) bool {
	if strings.TrimSpace(message.Text) != "" || strings.TrimSpace(message.Context) != "" {
		return true
	}
	for _, segment := range message.Segments {
		if strings.TrimSpace(segment.Text) != "" || strings.TrimSpace(segment.Data) != "" {
			return true
		}
	}
	return false
...
func upgradeLegacyAgentContent(message legacyMessage) []AgentContentPart {
	...
	if len(message.Segments) == 0 {
		content = append(content, AgentContentPart{Kind: AgentContentKindText, Text: firstNonEmptyString(message.Text, message.Context), Signature: "", ToolUse: nil})
	} else {
		for _, segment := range message.Segments {
			switch segment.Type {
