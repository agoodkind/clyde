// Package mcpserver: stdout serialization wrapper.
//
...
// JSON-RPC frames to the same os.Stdout. Some upstream paths (RequestSampling,
// ListRoots, RequestElicitation) write directly to the session writer without
// going through writeResponse, so they bypass the upstream writeMu. Two
// concurrent writes can interleave at the byte level, and the receiver sees
// torn JSON. Wrapping stdout in a writer that locks on every Write call
// serializes every byte regardless of the upstream code path.
//
