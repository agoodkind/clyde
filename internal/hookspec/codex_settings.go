...
func marshalCodexHookInstalls(existing []byte, installs []RegisteredInstall, signatures [][]string, clydeBin string, settingsPath string) ([]byte, error) {
	...
	base = removeCodexCommandHookGroups(base, codexCommandSignatures(signatures, clydeBin))
...
func removeCodexCommandHookGroups(input string, commands []string) string {
	if len(commands) == 0 || strings.TrimSpace(input) == "" {
		return input
	}
	...
	for index := 0; index < len(lines); {
		...
		if codexGroupContainsCommand(lines[index:end], commands) {
			index = end
			continue
		}
...
func codexGroupContainsCommand(lines []string, commands []string) bool {
...
func codexCommandSignatures(signatures [][]string, clydeBin string) []string {
