...
func runReorientBeforeCompact(ctx context.Context, env RunEnvironment) error {
	...
	eventName := input.eventName()
	if eventName != EventPreCompact && eventName != EventCursorPre {
		return nil
	}
	if env.Reorient == nil {
...
func runReorientAfterCompact(ctx context.Context, env RunEnvironment) error {
	...
	if input.eventName() != EventSessionStart || !input.compactSource() {
		return nil
	}
	if env.SnapshotStore == nil {
	...
	if err := writeAdditionalContext(env.Output, client, EventSessionStart, body); err != nil {
