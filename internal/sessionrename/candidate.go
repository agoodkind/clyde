...
func Candidate(ctx context.Context, input CandidateInput) (CandidateResult, error) {
	...
	if fallbackErr != nil {
		return CandidateResult{}, fmt.Errorf("sessionrename: llm path failed (%w) and transcript fallback failed: %w", llmErr, fallbackErr)
	}
...
func llmCandidate(ctx context.Context, input CandidateInput, redacted string) (CandidateResult, error) {
	...
	if validateErr != nil {
		return CandidateResult{}, fmt.Errorf("sessionrename: llm candidate %q rejected: %w", rawCandidate, validateErr)
	}
...
func transcriptCandidate(input CandidateInput, redacted string) (CandidateResult, error) {
	...
	if validateErr != nil {
		return CandidateResult{}, fmt.Errorf("sessionrename: transcript candidate %q rejected: %w", candidate, validateErr)
	}
	return CandidateResult{
		Name:   accepted,
