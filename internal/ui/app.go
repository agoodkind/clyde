...
func (a *App) handleDetailsLoaded(d detailsLoaded) {
	...
	if d.err != nil {
		if hadCache {
			// Preserve the prior cached stats so a transient RPC error
			// after a successful prior load does not flicker the
			// detail panel back to "loading...".
			...
			d.detail.TranscriptStatsLoaded = priorLoaded
		} else {
			if d.detail.TranscriptStatsStatus == "" {
				d.detail.TranscriptStatsStatus = fmt.Sprintf("failed: %v", d.err)
			}
...
func (a *App) loadDetailAsync(sess *session.Session) {
	...
	go func() {
		...
		a.postInterrupt(detailsLoaded{name: name, gen: gen, detail: detail, err: err})
	}()
}
