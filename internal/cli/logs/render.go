...
func writePurgeTable(writer io.Writer, result PurgeResult) error {
	if _, err := fmt.Fprintf(writer, "State root: %s\n", result.StateRoot); err != nil {
		return writeTableError("write state root line", err)
	}
	if _, err := fmt.Fprintf(writer, "Mode: %s\n", result.Mode); err != nil {
		return writeTableError("write mode line", err)
	}
	if _, err := fmt.Fprintf(writer, "Deleted files: %d (%s)\n", result.DeletedFiles, formatBytes(result.DeletedBytes)); err != nil {
		return writeTableError("write deleted summary", err)
	}
	if _, err := fmt.Fprintf(writer, "Skipped entries: %d\n", len(result.SkippedPaths)); err != nil {
		return writeTableError("write skipped count", err)
	}
	if _, err := fmt.Fprintf(writer, "Errors: %d\n", len(result.Errors)); err != nil {
		return writeTableError("write errors count", err)
	}
	if _, err := fmt.Fprintf(writer, "Duration: %d ms\n", result.DurationMS); err != nil {
		return writeTableError("write duration", err)
	}
	if err := writePurgePathSection(writer, "Legacy roots", result.LegacyRoots); err != nil {
		return err
	}
	if err := writePurgePathSection(writer, "Deleted paths", result.DeletedPaths); err != nil {
		return err
	}
	if err := writePurgePathSection(writer, "Skipped paths", result.SkippedPaths); err != nil {
		return err
	}
	if err := writePurgePathSection(writer, "Errors", result.Errors); err != nil {
		return err
	}
	return nil
}

func writePurgePathSection(writer io.Writer, header string, items []string) error {
	if len(items) == 0 {
		return nil
	}
	if _, err := fmt.Fprintf(writer, "\n%s:\n", header); err != nil {
		return writeTableError("write purge section header", err)
	}
	for _, item := range items {
		if _, err := fmt.Fprintf(writer, "  - %s\n", item); err != nil {
			return writeTableError("write purge section row", err)
		}
	}
	return nil
}
