...
// swaps the adapter model registry and every MITM proxy's provider routing, then
// advances the current-config pointer. If any component apply fails it returns
// the error WITHOUT advancing the pointer, so the caller falls back to the
...
// for that set, so this performs only the two atomic swaps.
