...
// NewConversationIndex builds a disk-backed conversation index with the provider
// parsers wired in. It serves callers outside the daemon worker, such as the
// scalar-backfill CLI command, that need the same derived records the daemon
// serves without standing up the full daemon.
