...
func TestMITMListenerSurvivesInheritanceHandoff(t *testing.T) {
	...
	parentProxy, err := mitm.NewProxy(config.MITMConfig{}, log, parentLis)
	if err != nil {
	...
	// answers a fresh request on the same address.
	childProxy, err := mitm.NewProxy(config.MITMConfig{}, log, childLis)
	if err != nil {
