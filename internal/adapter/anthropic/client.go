...
// errors are wrapped with context and emitted on the SSE concern.
func (t *captureTee) Read(p []byte) (int, error) {
	...
	slog.Warn("anthropic.sse.tee_read_failed", "concern", "adapter.providers.anthropic.sse", "subcomponent", "anthropic",
		"err", err.Error(),
	)
