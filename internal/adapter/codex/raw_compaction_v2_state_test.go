...
func TestRawResponsesCompactionV2RegistryFencesStaleLeaseAfterRelease(t *testing.T) {
	registry := NewRawResponsesCompactionV2Registry(nil)
	if !registry.Arm("s", "encrypted", "transcript") {
		t.Fatal("Arm() = false")
	}
	_, firstGeneration, ok := registry.Reserve("s", "encrypted")
	if !ok {
		t.Fatal("first Reserve() = false")
	}
	registry.Release("s", "encrypted", firstGeneration)
	_, secondGeneration, ok := registry.Reserve("s", "encrypted")
	if !ok {
		t.Fatal("second Reserve() = false")
	}
	if secondGeneration == firstGeneration {
		t.Fatalf("second generation = %d, want distinct from first", secondGeneration)
	}
	if registry.Complete("s", "encrypted", firstGeneration) {
		t.Fatal("stale Complete() removed second lease")
	}
	registry.Release("s", "encrypted", firstGeneration)
	if _, _, ok := registry.Reserve("s", "encrypted"); ok {
		t.Fatal("stale Release() changed second lease")
	}
	if !registry.Complete("s", "encrypted", secondGeneration) {
		t.Fatal("second Complete() = false")
	}
}
