...
func TestRawResponsesCompactionPreservesSSEFramesWithNullIntegerFields(t *testing.T) {
	itemDone, completed := rawCompactionSSEFramesForTest(
		`{"id":"msg-1","type":"message","role":"assistant","content":[{"type":"output_text","text":"summary"}]}`,
		0,
		10,
		11,
	)
	for _, testCase := range []struct {
		...
		{name: "null output index", body: strings.Replace(itemDone, `"output_index":0`, `"output_index":null`, 1) + completed},
		{name: "null sequence number", body: strings.Replace(itemDone, `"sequence_number":10`, `"sequence_number":null`, 1) + completed},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if string(got) != testCase.body {
				t.Fatalf("malformed SSE frame changed:\n got: %s\nwant: %s", got, testCase.body)
			}
		})
	}
}
