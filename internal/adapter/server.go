...
func New(ctx context.Context, cfg config.AdapterConfig, logging config.LoggingConfig, deps Deps, log *slog.Logger) (*Server, error) {
	...
	if cfg.DirectOAuth {
		s.initDirectOAuth(ctx, cfg, deps, log, max)
	}
	s.mux = s.routes()
	return s, nil
}

// initDirectOAuth wires up the OAuth manager, Anthropic client, and
// Anthropic provider when direct OAuth is configured. Extracted from
// [New] to satisfy the funlen lint budget.
func (s *Server) initDirectOAuth(ctx context.Context, cfg config.AdapterConfig, deps Deps, log *slog.Logger, maxConcurrent int) {
