...
// caValidity is the lifetime applied to a freshly minted Cursor MITM CA.
...
// pemBlockKind is the supported set of PEM block headers for the Cursor MITM
...
// loadOrCreateCertAuthority loads a Cursor MITM CA from disk when both the
...
func loadOrCreateCertAuthority(certPath, keyPath string, now func() time.Time) (*cursorCertAuthority, error) {
	if certPath == "" {
		err := errors.New("cursor mitm ca cert path is empty")
		slog.Warn("mitm.cursor.tls.ca_cert_path_empty", "err", err)
		return nil, err
	}
	if keyPath == "" {
		err := errors.New("cursor mitm ca key path is empty")
		slog.Warn("mitm.cursor.tls.ca_key_path_empty", "err", err)
		return nil, err
	}
	if now == nil {
		now = time.Now
	...
	if err != nil {
		slog.Warn("mitm.cursor.tls.ca_cert_stat_failed", "path", certPath, "err", err)
		return nil, fmt.Errorf("stat cursor mitm ca cert %q: %w", certPath, err)
	}
	keyExists, err := pathExists(keyPath)
	if err != nil {
		slog.Warn("mitm.cursor.tls.ca_key_stat_failed", "path", keyPath, "err", err)
		return nil, fmt.Errorf("stat cursor mitm ca key %q: %w", keyPath, err)
	}
	...
	case certExists != keyExists:
		...
		err := fmt.Errorf(
			"cursor mitm ca files inconsistent: cert exists=%t at %q, key exists=%t at %q",
			certExists, certPath, keyExists, keyPath,
		)
		slog.Warn("mitm.cursor.tls.ca_files_inconsistent",
...
func pathExists(path string) (bool, error) {
	...
	slog.Warn("mitm.cursor.tls.path_stat_failed", "path", path, "err", err)
	return false, fmt.Errorf("os.Stat: %w", err)
}

func loadExistingCertAuthority(certPath, keyPath string) (*cursorCertAuthority, error) {
	certPEM, err := os.ReadFile(certPath)
	if err != nil {
		slog.Warn("mitm.cursor.tls.ca_cert_read_failed", "path", certPath, "err", err)
		return nil, fmt.Errorf("read cursor mitm ca cert %q: %w", certPath, err)
	}
	keyPEM, err := os.ReadFile(keyPath)
	if err != nil {
		slog.Warn("mitm.cursor.tls.ca_key_read_failed", "path", keyPath, "err", err)
		return nil, fmt.Errorf("read cursor mitm ca key %q: %w", keyPath, err)
	}

	certBlock, _ := pem.Decode(certPEM)
	if certBlock == nil || certBlock.Type != "CERTIFICATE" {
		err := fmt.Errorf("cursor mitm ca cert %q is not a PEM CERTIFICATE", certPath)
		slog.Warn("mitm.cursor.tls.ca_cert_pem_invalid", "path", certPath)
		return nil, err
	}
	cert, err := x509.ParseCertificate(certBlock.Bytes)
	if err != nil {
		slog.Warn("mitm.cursor.tls.ca_cert_parse_failed", "path", certPath, "err", err)
		return nil, fmt.Errorf("parse cursor mitm ca cert %q: %w", certPath, err)
	}
	if !cert.IsCA {
		err := fmt.Errorf("cursor mitm ca cert %q is not a CA (IsCA=false)", certPath)
		slog.Warn("mitm.cursor.tls.ca_cert_not_a_ca", "path", certPath)
		return nil, err
	}

	keyBlock, _ := pem.Decode(keyPEM)
	if keyBlock == nil {
		err := fmt.Errorf("cursor mitm ca key %q is not PEM-encoded", keyPath)
		slog.Warn("mitm.cursor.tls.ca_key_pem_invalid", "path", keyPath)
		return nil, err
	}
	key, err := decodeRSAPrivateKey(keyBlock)
	if err != nil {
		slog.Warn("mitm.cursor.tls.ca_key_parse_failed", "path", keyPath, "err", err)
		return nil, fmt.Errorf("parse cursor mitm ca key %q: %w", keyPath, err)
	}
	...
	if _, err := tls.X509KeyPair(certPEM, keyPEM); err != nil {
		slog.Warn("mitm.cursor.tls.ca_pair_mismatch",
			"cert_path", certPath, "key_path", keyPath, "err", err,
		)
		return nil, fmt.Errorf(
			"cursor mitm ca cert %q and key %q do not match: %w",
...
func decodeRSAPrivateKey(block *pem.Block) (*rsa.PrivateKey, error) {
	...
	case pemBlockKindPKCS1:
		...
		if err != nil {
			slog.Warn("mitm.cursor.tls.ca_key_pkcs1_parse_failed", "err", err)
	...
	case pemBlockKindPKCS8:
		...
		if err != nil {
			slog.Warn("mitm.cursor.tls.ca_key_pkcs8_parse_failed", "err", err)
			return nil, fmt.Errorf("pkcs8: %w", err)
		}
		key, ok := parsed.(*rsa.PrivateKey)
		if !ok {
			slog.Warn("mitm.cursor.tls.ca_key_pkcs8_wrong_type", "type", fmt.Sprintf("%T", parsed))
			return nil, fmt.Errorf("pkcs8 key is %T, want *rsa.PrivateKey", parsed)
		}
		return key, nil
	default:
		slog.Warn("mitm.cursor.tls.ca_key_unsupported_pem_type", "type", block.Type)
		return nil, fmt.Errorf("unsupported PEM block type %q", block.Type)
	}
}

func createCertAuthority(certPath, keyPath string, now func() time.Time) (*cursorCertAuthority, error) {
	...
	template := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "Clyde Cursor MITM CA"},
	...
	slog.Info("mitm.cursor.tls.ca_persisted",
		"cert_path", certPath,
		"key_path", keyPath,
...
func persistCAFiles(certPath, keyPath string, certPEM, keyPEM []byte) error {
	for _, dir := range []string{filepath.Dir(certPath), filepath.Dir(keyPath)} {
		if err := os.MkdirAll(dir, caDirFileMode); err != nil {
			slog.Warn("mitm.cursor.tls.ca_dir_failed", "path", dir, "err", err)
			return fmt.Errorf("create cursor mitm ca dir %q: %w", dir, err)
		}
	}
	...
	if err := writeFileExclusive(certPath, certPEM, caCertFileMode); err != nil {
		slog.Warn("mitm.cursor.tls.ca_cert_write_failed", "path", certPath, "err", err)
		return fmt.Errorf("write cursor mitm ca cert %q: %w", certPath, err)
	}
	if err := writeFileExclusive(keyPath, keyPEM, caKeyFileMode); err != nil {
		...
		slog.Warn("mitm.cursor.tls.ca_key_write_failed", "path", keyPath, "err", err)
		return fmt.Errorf("write cursor mitm ca key %q: %w", keyPath, err)
	}
	return nil
}
...
func writeFileExclusive(path string, data []byte, mode os.FileMode) error {
	...
	if err != nil {
		slog.Warn("mitm.cursor.tls.ca_open_excl_failed", "path", path, "err", err)
		return fmt.Errorf("os.OpenFile: %w", err)
	}
	if _, err := f.Write(data); err != nil {
		_ = f.Close()
		slog.Warn("mitm.cursor.tls.ca_write_failed", "path", path, "err", err)
		return fmt.Errorf("file.Write: %w", err)
	}
	if err := f.Close(); err != nil {
		slog.Warn("mitm.cursor.tls.ca_close_failed", "path", path, "err", err)
...
// cursorCA returns the persisted Cursor MITM CA loaded during proxy
...
func (p *Proxy) cursorCA() (*cursorCertAuthority, error) {
	...
	if p.ca == nil {
		err := errors.New("cursor mitm ca not initialized")
		slog.Warn("mitm.cursor.tls.ca_missing", "err", err)
		return nil, err
	}
	return p.ca, nil
}
