...
func TestTransparentRawTLSInterceptsUnclaimedHostAndCaptures(t *testing.T) {
	const claimedHost = "api2direct.cursor.sh"
	const unclaimedHost = "updates.example.test"
	const requestID = "req_transparent_unclaimed_capture_123"
	const sentinel = "TRANSPARENT_UNCLAIMED_SENTINEL"
	requestBody := []byte("prefix " + sentinel + " suffix")
	upstreamBody := []byte(`{"ok":true,"transport":"transparent-unclaimed"}`)
	upstream := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/unclaimed/transparent" {
			t.Errorf("path = %q", r.URL.Path)
		}
		body, err := io.ReadAll(r.Body)
		if err != nil {
	...
	defer h2Client.Close()
	req, err := http.NewRequest(http.MethodPost, "https://"+unclaimedHost+"/v1/unclaimed/transparent", bytes.NewReader(requestBody))
	if err != nil {
		t.Fatalf("build request: %v", err)
	}
	...
	roundTripCtx, cancelRoundTrip := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancelRoundTrip()
	resp, err := h2Client.RoundTrip(req.WithContext(roundTripCtx))
	if err != nil {
		t.Fatalf("h2 round trip: %v", err)
	}
	gotBody, err := io.ReadAll(resp.Body)
